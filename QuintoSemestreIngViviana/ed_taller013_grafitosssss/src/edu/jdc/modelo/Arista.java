...
import edu.jdc.control.Flechas;
...
public class Arista {
	...
	//metodo constructor
	public Arista(String pesoExterno, Point p1, Point p2, GenerarNodo inicio, GenerarNodo finn) {
		peso = pesoExterno;
		puntoIni = p1;
		puntoFin = p2;
		nodoIni = inicio;
	...
	private void cargarConstantes() {
		colorArista = Interfaces.ARISTA_COLOR;
		colorFlecha = Interfaces.FLECHA_COLOR;
		diametro = Interfaces.DIAMETRO;
		tamanoPunta = Interfaces.FLECHA_TAMANIO_PUNTA;
		grafoDirigido = Interfaces.GRAFO_DIRIGIDO;
		posicionPunta = Interfaces.POSICION_PUNTITA;
	}
	
	private void diseñarFlecha(int x0, int y0, int x1, int y1, int largoPuntas, Graphics g) {
		double diferencialY = y1 - y0;
		double diferencialX = x1 - x0;
		
		//Se calcula el ángulo de inclinación de la línea que conecta los dos puntos utilizando la función Math.atan2
	...
	private void relacion(Graphics g) {
	...
	public void dibujar(Graphics g) {
		...
		if (grafoDirigido) {
			...
			int finX = (int)(puntoFin.getX());
			int finY = (int)(puntoFin.getY());
			Point puntoFlecha = Flechas.encontrarPunto(iniX, iniY, finX, finY,posicionPunta);
			int puntoFlechaX = (int)(puntoFlecha.getX());
			int puntoFlechaY = (int)(puntoFlecha.getY());
			diseñarFlecha(iniX,iniY, puntoFlechaX, puntoFlechaY,tamanoPunta,g);
	...
	public GenerarNodo getNodoIni() {
		return nodoIni;
	}


	public void setNodoIni(GenerarNodo nodoIni) {
		this.nodoIni = nodoIni;
	}


	public GenerarNodo getNodoFin() {
		return nodoFin;
	}


	public void setNodoFin(GenerarNodo nodoFin) {
